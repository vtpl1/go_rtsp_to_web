...
// RTSPServer func
func RTSPServer() {
	utils.Logger.Infoln("Server RTSP start")
	l, err := net.Listen("tcp", datamodel.Storage.ServerRTSPPort())
	if err != nil {
...
func RTSPServerClientHandle(conn net.Conn) {
	...
	for {
		...
		utils.Logger.Debugln(string(buf[:n]))
		if err != nil {
			utils.Logger.Errorln(err.Error())
			return
		}
		...
		case OPTIONS:
			...
			if !datamodel.Storage.StreamChannelExist(uuid, channel) {
			...
			if !webstreamer.RemoteAuthorization("RTSP", uuid, channel, token, conn.RemoteAddr().String()) {
				utils.Logger.Errorln(webstreamer.ErrorStreamNotFound.Error())
			...
			datamodel.Storage.StreamChannelRun(uuid, channel)
			err = RTSPServerClientResponse(uuid, channel, conn, 200, map[string]string{"CSeq": strconv.Itoa(cSEQ), "Public": "DESCRIBE, SETUP, TEARDOWN, PLAY"})
			if err != nil {
				return
			}
		case SETUP:
			...
			in = in + 2
		case DESCRIBE:
			sdp, err := datamodel.Storage.StreamChannelSDP(uuid, channel)
			if err != nil {
				utils.Logger.Errorln(err.Error())
				return
			}
		...
		case TEARDOWN:
			...
			return
		default:
			utils.Logger.Debugln("stage bad", stage)
		}
	}
}
...
func parsecSEQ(buf []byte) int {
	return vdk.StringToInt(vdk.StringInBetween(string(buf), "CSeq: ", "\r\n"))
}
...
func RTSPServerClientResponse(uuid string, channel string, conn net.Conn, status int, headers map[string]string) error {
	...
	utils.Logger.Debugln(builder.String())
...
func parseStreamChannel(buf []byte) (string, string, string, error) {
	...
	uri := vdk.StringInBetween(string(buf), " ", " ")
	u, err := url.Parse(uri)
	if err == nil {
...
func RTSPServerClientPlay(uuid string, channel string, conn net.Conn) {
	cid, _, ch, err := datamodel.Storage.ClientAdd(uuid, channel, datamodel.RTSP)
	if err != nil {
		utils.Logger.Errorln(err.Error())
		return
	}
	defer func() {
		datamodel.Storage.ClientDelete(uuid, cid, channel)
		utils.Logger.Infoln("Client offline")
		err := conn.Close()
		if err != nil {
			utils.Logger.Errorln(err.Error())
		}
