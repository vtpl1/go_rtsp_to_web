...
type Foo struct {
	IntField    int    `zap:"int_field"`
	StringField string `zap:"string_field"`
	Optional    string `zap:"optional, omitempty"`
}

func (f *Foo) MarshalLogObject(enc zapcore.ObjectEncoder) error {
	enc.AddInt("int_field", f.IntField)
	enc.AddString("string_field", f.StringField)
	if f.Optional != "" {
		enc.AddString("optional", f.Optional)
	}
	return nil
}
...
func StreamServerRunStream(streamID string, channelID string, opt *ChannelST) (int, error) {
	...
	*/
	...
	utils.Logger.Info("Success connection RTSP")
	var ProbeCount int
	var ProbeFrame int
...
func StreamServerRunStreamRTMP(streamID string, channelID string, opt *ChannelST) (int, error) {
	...
	utils.Logger.Infow("Success connection RTSP", "stream", streamID,
		"channel", channelID)
	var ProbeCount int
	var ProbeFrame int
	var ProbePTS time.Duration
